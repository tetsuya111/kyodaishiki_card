...
class CSM:
	...
	TAG_SEQ=","
	ENCODING="utf8"
	def __init__(self,memo="",comment="",tags=[],date=""):
		self.id=_util.hash(memo)
		self.memo=memo
	...
	def dump(self,mode="mctd"):
		...
		mode=mode.upper()
		if "A" in mode:
			return self.dump("MCTDF")
		if "M" in mode:
			res+=self.memo+CSM.SEQ
		if "C" in mode:
			res+=self.comment+CSM.SEQ
		if "T" in mode:
			res+=",".join(self.tags)+CSM.SEQ
		if "D" in mode:
			res+=self.date+CSM.SEQ
		if "F" in mode:
			res+=self.fname
		return res.rstrip()
...
#{"name":"* ...","tags":["- ...","- ...",...]}
	...
	def getData(self):
		def getName(line):
			return line[1:] if line and line[0] == TOT.NAME_HEAD else line
		def getChilds(lines):
			return list((map(lambda line:line[1:] if line and line[0]==TOT.CHILD_HEAD else line,lines)))
		lines=self.comment.split("\n")
		return (getName(lines[0]) if lines else "",getChilds(lines[1:]) if len(lines)>1 else [])
	@property
	def name(self):
		return	self.data[0]
	@property
...
#allTags is for expandReg.
			...
			while i<lenS:
				if s[i]==Logic.NOT and not data:
					...
					if end==-1:
						raise Exception('left of "(" is nothing. {0}'.format(s[i:]))
					g.not_=not_
