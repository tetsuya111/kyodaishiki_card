...
"""
	...
	def __init__(self,dname):
		...
		if os.path.exists(self.dataFile):
			with open(self.dataFile,"r",encoding=self.ENCODING) as f:
	...
	def save(self):
		if not os.path.exists(self.dname):
			os.mkdir(self.dname)
		with self.mutex:
			with open(self.dataFile,"w",encoding=self.ENCODING) as f:
...
#getPath of dname specified by dbid
	...
	def dumpDB(self):
		...
		for dbIdx in self.listDB():
			try:
				db=self.select(dbIdx.getID(self.text))
			except Exception as e:
				print(e,dbIdx.getID(self.text))
				continue
			if db:
				yield db	
...
#append DB.
		...
		if not os.path.exists(dbDname):
			os.mkdir(dbDname)
	def remove(self,dbid,trush=False):
#Remove DB specified by dbid
		dbidIdx=self.find(dbid)
		if super().remove(dbidIdx):	#if index is removed
			try:
				if trush:
					winshell.delete_file(os.path.join(self.dname,dbid))	#remove data
				else:
