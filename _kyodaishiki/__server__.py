...
#APPEND <csms>...  #csms is json.
	...
	def handle(self):
		...
		while True:
			...
			query=_util.Query.read(data)
			self.server.log_connection(self.client_address[0],{"data":data,"client_address":self.client_address})
			#print("Data",data)
			if query.command in CardHandler.Command.SEARCH:
				self.search(userid,query.args,self.wfile)
			elif query.command in CardHandler.Command.TAG:
				self.printtag(userid,query.args,self.wfile)
			elif query.command in CardHandler.Command.TOT:
				self.getTOTs(userid,query.args,self.wfile)
			elif query.command in CardHandler.Command.VIM:
				self.vim(userid,query.args,self.wfile)
			elif query.command in CardHandler.Command.WRITE:
				self.write(userid,data,self.wfile)
#WRITE <csm>
			elif query.command in CardHandler.Command.CSM:
				data=data.split(" ",1)
				command=data[0].upper()
				data=data[1] if len(data)>1 else list()
				if command in CardHandler.Command.APPEND:
...
#mode is "R" or "W"
		if not self.server.checkUser(mode.upper(),userid):
			...
			return True
		return False
	def search(self,userid,args,output):
#write res to output as encoded.
		if self.blocked(userid,"R",output):
			return
		try:
			args=docopt.docopt(_util.Docs.Server.Card.SEARCH,args)
		...
		asfile=args["af"] or args["asfile"]
		cards=self.server.search(memo=memo,tags=tags)
		csms=__data__.CSM.make(self.server.text,cards)
		if comment:
			csms=filter(lambda csm:comment in csm.comment,csms)
		if asfile:
			res=list(map(lambda csm:csm.fname,csms))
		else:
			res=list(map(str,csms))
		res=json.dumps(res)
		output.write(("200 "+res).encode())
	def printtag(self,userid,args,output):
		if self.blocked(userid,"R",output):
			return
		try:
			args=docopt.docopt(_util.Docs.Server.Card.TAG,args)
		except SystemExit as e:
			print(e,file=output)
			return
		tag=args["<tag>"] or ""
		tags=filter(lambda tag__:re.search(tag,tag__),self.server.getTags())
		res="\n".join(tags)
		output.write(("200 "+res).encode())
	def getTOTs(self,userid,args,output):
		if self.blocked(userid,"R",output):
			return
		try:
			args=docopt.docopt(_util.Docs.Server.Card.TOT,args)
		except SystemExit as e:
			print(e,file=output)
			return
		tag=args["<tag>"] or ""
		...
		res=json.dumps(res)
		output.write(("200 "+res).encode())
	def vim(self,userid,args,output):
		if self.blocked(userid,"R",output):
			return
		try:
			args=docopt.docopt(_util.Docs.Server.Card.VIM,args)
		except SystemExit as e:
			print(e,file=output)
			return
		if args["l"] or args["list"]:
			...
			for csm in csms:
				...
				res+=("-"*50) + "\n"
			output.write(("200 "+res).encode())
	def write(self,userid,args,output):
...
#client for BaseServerHome
	...
	def getPort(self,dbid):
		with self.socketForHome() as __socket:
			...
			#con.sendall(query.encode())
			data=__utils__.recvall(__socket,blocked=True).decode("utf8","ignore")
			#print("Data",data)
		if not data:
			raise Exception("Can't connect [1] "+str(dbid))
		else:
			status,port=data.split(" ",1)
			if status[0]=="4":
				raise Exception(f"Can't connect [2] {dbid} : {port}")
			return int(port)
...
class BaseClientU(BaseClient):
	def __init__(self,userid,port):
		self.userid=userid
		host=getHost(userid) or "localhost"
...
#client for CardHandler
	...
	def requestCSM(self,command,data):
		return self.request(CardHandler.Command.CSM[0]+" "+command,data)
	def search(self,memo="",tags=[]):
		...
		if tags:
			...
			query+=f"-t {tags} "
		query=query.strip()
		data=self.request(CardHandler.Command.SEARCH[0],query)
		if not data:
			return
		data=data.split(" ",1)
		if data[0][0]!="2":
	...
	def getTags(self,tag=""):
		...
		if tag:
			query+=f" -t {tag} "
		query=query.strip()
		data=self.request(CardHandler.Command.TAG[0],query)
		if not data or data[0]!="2":
			return []
		return data.split(" ",1)[1].split("\n")
	def getTOTs(self,tag=""):
		query=""
		if tag:
			query+=f" -t {tag} "
		query=query.strip()
		data=self.request(CardHandler.Command.TOT[0],query)
		if not data or data[0]!="2":
			return []
		data=data.split(" ",1)[1]
		if data:
	...
	def write(self,memo,comment=str(),tags=list(),date=str()):
		...
		data=self.request(CardHandler.Command.WRITE[0],str(__data__.CSM(memo,comment,tags,date)))
		if not data:
			return False
		data=data.split(" ")
		if data[0][0]=="2":
	...
	def appendCSMs(self,csms,override=False):
		...
		return self.requestCSM(CardHandler.Command.APPEND[0],json.dumps(data))
