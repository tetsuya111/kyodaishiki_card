...
#		start() : override of begin().process enterBat and exitBat.
	...
	def execQuery(self,query,output):
		...
		if query.command in Command.QUIT:
			...
			if args["file"]:
				...
				return self.beginf(fname,output)
			...
			if args["stdin"]:
				...
				for i in range(max(lenArgs)):
					args__=list(map(lambda arg,length:arg[i]if i<length else arg[length-1],args,lenArgs))
			...
			try:
                            if type(sys.stdin) in (_pyio.StringIO,_pyio.BytesIO):
                                origin_stdin=sys.stdin
                                encoding="sjis"
                                tmp=tempfile.TemporaryFile(mode='w+',encoding=encoding)
                                tmp.write(origin_stdin.read())
                                tmp.seek(0)
                                sys.stdin=tmp
                            res=sp.check_output(" ".join(query.args),stdin=sys.stdin.fileno(),shell=True)
                            res=res.decode("sjis")
	...
	def beginf(self,fname,stdout=None):
		...
		with open(fname,"r") as f:
			return self.__begin__(f,stdout)
	def shell(self):
...
#manage db and client
	def execQuery(self,query,output):
		if query.command in Command.DB.SERVER:
			...
			pMode=args["<pMode>"].upper() if args["-p"] else "A"
			all_=args["-a"] or args["--all"]
			if args["-u"]:
				userid=args["<userid>"] 
