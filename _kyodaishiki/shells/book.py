...
class Shiori:
	...
	def search(self,memo="",tags=[],book_tags=[],title="",author="",p_from=0,p_until=10000):
		def get_titles_by_tags(book_tags):
			return map(lambda card:card.memo(self.db.text),\
				self.db.search(tags=[Tag.IS_BOOK,*book_tags]))
		if book_tags:
			book_titles=list(map(lambda title:f".*{title}.*",get_titles_by_tags(book_tags)))
		else:
			book_titles=[]
		if author:
			book_titles.extend(getBookTitles(self.db,author))
		if title:
		...
		for card in self.db.search(memo,(self.TAG,*tags)):
			...
			p_l=Attr.get(tags,Attr.P_L)
			p=p_l[0].split("_")[0] if p_l else 0
			try:
				p=int(p)
			except:
				p=0
			if select2.NOT in tags:
				continue
			if p_from <= p < p_until:
				yield card
	def getTitles(self):
		explored=[]
		for titles in map(lambda card:Attr.get(card.tags(self.db.text),Attr.TITLE),sorted(self.search(),key=lambda card:card.date)):
			if not titles:
				continue
			title=titles[0]
		...
		#return set(reduce(lambda a,b:a+b,map(lambda card:Attr.get(card.tags(self.db.text),Attr.TITLE),sorted(self.search(),key=lambda card:card.date))))
...
def appendTitleOfAuthor(db,author,title):
...
class Book(__data__.CSM):
	...
	def write(db,output=sys.stdout):
		...
		appendTitleOfAuthor(db,author,title)
...
def _getRelated(csms,relatedTags=[]):
	res={}
	for csm in csms:
...
def getRelated(db,tags=[],title="",author="",relatedTags=[]):
	csms=__data__.CSM.make(db.text,search(db,tags,title,author))
	return _getRelated(csms,relatedTags=relatedTags)

class Docs:
	class DB:
		ALTER="""
		Usage:
			alter (b|book) [(-t <tags>)] [(-T <title>)] [(-a <author>)]
		...
		Usage:
			...
			search_card (sh|shi|shiori) [(-m <memo>)] [(-t <tags>)] [(--bt <book_tags>)] [(-T <title>)] [(-a <author>)] [(--pf <p_from>)] [(--pu <p_until>)] [(-F <from>)] [(-U <until>)]
		...
		Usage:
			...
			search (sh|shi|shiori) [(-m <memo>)] [(-t <tags>)] [(--bt <book_tags>)] [(-T <title>)] [(-a <author>)] [(--pf <p_from>)] [(--pu <p_until>)] [(-p <pMode>)] [(-r|--random)]
		...
		Usage:
			...
			list (ti|title) (sh|shi|shiori) [(-T <title>)] [(-r|--random)] [(--recent)]
		...
		Usage:
			...
			dump (w|want) [(-T <titles>)] [(-t <tags>)] [(-m <memo>)]
		"""
		GET="""
		Usage:
			get (bn|bookname) (re|reading) [(-t <tags>)]
			get (bn|bookname) [(-U | --unique)] [<searchShioriArgs>...]
			get (re|related) [(-t <tags>)] [(-T <title>)] [(-a <author>)] [(--rt <relatedTags>)]
		"""
		GET_BOOKNAME="""
		Usage:
			getbn [<searchShioriArgs>...]
		"""
		HELP="""
			append
			search
	...
	def execQuery(self,query,output):
		if query.command in Command.HELP:
			...
			#dump (s|saw|r|read) [(-o output)] [(-t <tags>)] [(-T <title>)] [(-a <author>)] 
				...
				return category.DBShell(self.db,None,tag=Tag.WANT).execQuery(query,output)
			...
			if args["b"] or args["book"]:
				...
				if args["-t"]:
					tags=args["<tags>"].upper().split(",")
				elif self.environ.get(EnvKey.SHIORI_TAGS):
					tags=self.environ.get(EnvKey.SHIORI_TAGS).upper().split(",")
				else:
					tags=[]
			...
			try:
				args=docopt.docopt(Docs.DB.GET,query.args)
			except SystemExit as e:
				print(e)
				return
			if args["bn"] or args["bookname"]:
					if args["re"] or args["reading"]:
						tags=args["<tags>"].upper().split(",") if args["-t"] else []
						tags=(*tags,Tag.READING)
						tags=",".join(tags)
						data=self.get_bookname(("/t",tags,"/T",".+"),output)
						for card,title in data:
							card_title=card.memo(self.db.text)
							p_l=util.Attr.get(card.tags(self.db.text),Attr.P_L)
							p_l=p_l[0] if p_l else ""
							print(f"* {card_title}\t-> {title} ; P_L:{p_l}",file=output)
					else:
						searchArgs=args["<searchShioriArgs>"] or ("/T",".+")
						data=self.get_bookname(searchArgs,output)
						unique=args["-U"] or args["--unique"]
						explored=[]
						for card,title in data:
							if unique:
                                                            hashed_title=hash(title)
                                                            if hashed_title in explored:
                                                                continue
                                                            explored.append(hashed_title)
							card_title=card.memo(self.db.text)
							#print(f"* {title}\t-> {card_title}",file=output)
							print(f"* {card_title}\t-> {title}",file=output)
			elif args["re"] or args["related"]:
				tags=args["<tags>"].upper().split(",") if args["-t"] else []
				title=args["<title>"] if args["-T"] else ""
				author=args["<author>"] if args["-a"] else ""
				...
				#self.db.append("Related",comment.rstrip(),["related",Tag.WANNA_READ,"WANT",*related],override=override)
				csm=__data__.CSM("Related",comment.strip(),["related",Tag.WANNA_READ,"WANT",*related])
				print(csm,file=output)

		else:
			return super().execQuery(query,output)
	def get_bookname(self,args__,output):
		try:
			args=docopt.docopt(Docs.DB.GET_BOOKNAME,args__)
		except SystemExit as e:
			print(e)
			return
		searchArgs=args["<searchShioriArgs>"] or []
		searchArgs=list(util.arg_replace(searchArgs))
		cards=self.execQuery(__shell__.Query(("search_card","shiori",*searchArgs)),output)
		if not cards:
			return []
		cards=filter(lambda card:select2.NOT not in card.tags(self.db.text),cards)
		def getTitles():
			for card in cards:
				data=Attr.parse(card.tags(self.db.text))
				for title in data.get(Attr.TITLE,[]):
					yield (card,title)
		return getTitles()
		
	def search_card(self,args__,output):
		try:
			args=docopt.docopt(Docs.DB.SEARCH_CARD,args__)
		except SystemExit as e:
			print(e)
			return
		#search_card (b|book) [(-t <tags>)] [(-T <title>)] [(-a <author>)] 
		...
		if args["b"] or args["book"]:
			...
			memo=args["<memo>"] or ""
			tags=args["<tags>"].upper().split(",") if args["-t"] else []
			book_tags=args["<book_tags>"].upper().split(",") if args["--bt"] else []
			title=args["<title>"] or self.environ.get(EnvKey.SHIORI_TITLE)
			if title:
				title=f".*{title}.*"
			author=args["<author>"] or ""
			...
			return filter(lambda card:from_ <= util.to_datetime(card.date) < until,Shiori(self.db).search(memo,tags,book_tags,title,author,p_from,p_until))
		else:
			return super().search_card(args__,output)
