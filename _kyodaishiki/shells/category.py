...
class Command(__shell__.Command):
	DUMP=("D","DUMP")
			...
			def dump_get_cols():
				for csm in __data__.CSM.make(self.db.text,self.db.search(memo,tags)):
					...
					for c in l.searchRec(titles):
						for col in c.cols:
							col=util.Category.clean_as_tag(col)
							if col:
								yield col
...
class DUShell(__shell__.BaseHomeShell):
	...
	def select(self,args,output):
		...
		all_alias=os.path.join(self.home.dname,shell.ALL_ALIAS_TXT)
