...
from _kyodaishiki._util import to_datetime,_to_datetime
import docopt
import os
	...
	class Home:
		...
		def make(srcDname,dstDname):
			...
			try:
				...
				return None
			ENCODING="utf8"
			with open(os.path.join(dstDname,Data.Home.DB_TAG_FILE),"w",encoding=ENCODING) as f:
...
def arg_replace(args):
	for arg in args:
		n=re.match("/+",arg)
		n=len(n.group()) if n else 0
		yield re.sub("/"*n,"-"*n,arg)

def partitionArgs(l,cutunders=[]):
	for cutunder in cutunders:
		if cutunder in l:
			i=l.index(cutunder)
			if i+1 == len(l):
				return (l[:i],cutunder,[])
			return (l[:i],cutunder,l[i+1:])
	return (l,None,[])
...
class TagExpander:
	...
	def __init__(self,db):
		self.db=db
	def expandVars(self,tag,U):
		def getReplaced(base,src,U):
			for dst in self.db.getTagsRec([src],U):
				yield base.replace(self.VAR_F.format(src),dst)
		frontiers=[tag]
		while frontiers:
			now=frontiers.pop()
			var=re.search(self.VAR_F.format(".+"),now)
			if not var:
				yield now
			else:
				src=var.group()[1:-1]
				frontiers.extend(getReplaced(now,src,U))
	def expandReg(self,tag,U):
		allTags=list(map(lambda tagIdx:tagIdx.get(self.db.text),U))
		return __data__.Logic.expandReg(tag,allTags).split(__data__.Logic.OR)
	def expand(self,tag,U):
		for tag__ in self.expandReg(tag,U):
			for res in self.expandVars(tag__,U):
				yield res


_initial_missing=object()
def reduce(func,data,init=_initial_missing):
	data=iter(data)
	if init is _initial_missing:
		try:
			res=next(data)
...
class Docs:
	...
	Usage:
		...
		append csm <srcid> <dstid> (s|search) [(-O|--override)] [<searchArgs>...]
		...
		if query.command in _util.Command.HELP:
			...
			if args["data"]:
				...
				if args["csm"]:
					if args["s"] or args["search"]:
						...
						searchArgs=arg_replace(searchArgs)
						cards=src.search_card(searchArgs,output)
					else:
						tags=args["<tags>"].upper().split(",") if args["-t"] else []
						memo=args["<memo>"] if args["-m"] else ""
						data=""
						cards=src.search(memo,tags)
					for csm in __data__.CSM.make(src.text,cards):
			...
			if args["home"]:
				...
				if os.path.exists(dname):
					winshell.delete_file(dname)
				def _():
					self.execQuery(__shell__.Query.read("csm rm .+"),output)
				if rm_csm:
                                    _()
				else:
                                    yn=input("Do you remove all csm files? (Y/N)") or ""
                                    yn=yn.upper()
                                    if yn == "Y":
                                        _()
				self.execQuery(__shell__.Query.read("copy home -d {0}".format(dname)),output)
				print(f"Backuped home to {dname}",file=output)
			else:
				...
				for dbid in list(dbids):
					...
					if self.homeDB.select(dstid):
						try:
							self.homeDB.remove(dstid,trush=True)
						except Exception as e:
							print("E",e)
					try:
			...
			from_=to_datetime(from_)
			until=to_datetime(until)
			text=""
			data=[]
			def count():
				for dbid__ in self.homeDB.getDBIDs(dbid):
					...
					cards=filter(lambda card:from_<=to_datetime(card.date)<until,cards)
					n=len(list(cards))
					yield (dbid__,n)
			...
			try:
				args=docopt.docopt(Docs.CLEAN,query.args)
			except SystemExit as e:
				print(e)
				return
			dbid=args["<dbid>"]
			for card in list(self.homeDB.search(dbid,[])):
				dbid__=card.getID(self.homeDB.text)
				db=self.homeDB.select(dbid__)
				db.clean()
				print("Cleaned",dbid__,file=output)
